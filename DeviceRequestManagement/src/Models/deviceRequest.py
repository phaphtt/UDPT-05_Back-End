...
class DeviceRequest:
    ...
    def insertInformation(self, idEmployee, requestName, type, reason, noteRequest):
        ...
        conn.close()
        return
    def updateInformationByITManager(self, id, idITManager, requestITRejectReason, requestStatus):
        conn = connectDatabase.connect()
        cursor = conn.cursor()
        procedure = 'UpdateDeviceRequestByITManager'
        cursor.callproc(procedure, [id, idITManager, requestITRejectReason, requestStatus,])
        cursor.close()
        conn.close()
        return
    def updateInformationByPresident(self, id, requestManagerRejectReason, requestStatus):
        conn = connectDatabase.connect()
        cursor = conn.cursor()
        procedure = 'UpdateDeviceRequestByPresident'
        cursor.callproc(procedure, [id, requestManagerRejectReason, requestStatus,])
        cursor.close()
        conn.close()
        return
...
def list_all_DeviceRequest():
    conn = connectDatabase.connect()
    cursor = conn.cursor()
    query = ('SELECT * FROM DeviceRequest.DeviceRequest WHERE active = 1')
